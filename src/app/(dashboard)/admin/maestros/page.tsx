...
import { MaestroForm } from "@/components/forms";
...
export default function MaestrosPage() {
  ...
  const handleDelete = async (id: string) => {
    if (confirm("¿Seguro que deseas eliminar este maestro?")) {
    ...
    <div className="p-6">
      <div className="flex justify-between items-center mb-6">
        <div>
          ...
          <p className="text-sm text-slate-500">Gestión de personal docente y fotografías</p>
        </div>
        <Dialog open={open} onOpenChange={setOpen}>
          ...
          <DialogContent>
            ...
            <MaestroForm onSuccess={() => { setOpen(false); fetchData(); }} />
          </DialogContent>
        </Dialog>
      </div>
      ...
      <Dialog open={editOpen} onOpenChange={setEditOpen}>
        <DialogContent>
          <DialogHeader><DialogTitle>Editar Maestro</DialogTitle></DialogHeader>
          {editData && <MaestroForm initialData={editData} onSuccess={() => { setEditOpen(false); fetchData(); }} />}
        </DialogContent>
      </Dialog>

      {/* Modal para que el Administrador cambie la foto del Maestro */}
